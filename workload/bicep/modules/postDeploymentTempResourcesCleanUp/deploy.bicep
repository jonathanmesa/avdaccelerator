...
param azureCloudName string = environment().name
...
// =========== //
var varPostDeploymentTempResuorcesCleanUpScriptArgs = '-deleteResources ${deleteResources} -subscriptionId ${subscriptionId} -serviceObjectsRgName ${serviceObjectsRgName}  -azureCloudEnvironment ${azureCloudName} -managmentVmName ${managementVmName} -Verbose' // -computeObjectsRgName ${computeObjectsRgName} -storageObjectsRgName ${storageObjectsRgName} -networkObjectsRgName ${networkObjectsRgName} -monitoringObjectsRgName ${monitoringObjectsRgName}'
